title 'Ensure that No EGRESS firewall rules allow 0.0.0.0/0 from any sources'
...
control "cap-gcp-#{control_id}-#{control_abbrev}" do
  impact 1.0
  title "[#{control_abbrev.upcase}] - Ensure that No EGRESS firewall rules allow 0.0.0.0/0 from any sources"

  tag "control_ids": ["NS18"]
